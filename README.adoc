...
`debug` returns a `with` function to pass your own callback, you can use this to do something else instead of just logging the value.

```javascript
const count = ref(0);

debug(count).with((value) => {
...
`debug` only works during development, it is removed from the production build.
