...
`watchEffect` is the direct replacement for `$effect`, `watchEffect` takes in a function and returns a cleanup function, it automatically tracks the dependencies of the function and re-runs the function when any of the dependencies change.

```javascript

import { ref, watchEffect } from 'svelte-reactivity';
...
`debug` is a utility function that logs the value of a `ref` whenever it changes. It returns a `with` function that can be used to pass your own callback that gets called instead of the default `console.log`.

```javascript
const count = ref(0);

debug(count); // init: 0, update: 1
